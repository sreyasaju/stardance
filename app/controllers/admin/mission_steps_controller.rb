module Admin
  class MissionStepsController < Admin::MissionsBaseController
    before_action :set_step, only: [ :update, :destroy ]

    def create
      step = @mission.steps.new(step_params.merge(position: next_position))
      step.save!
      redirect_to admin_mission_path(@mission.slug), notice: "Step added."
    end
    ...
    def update
      ...
      redirect_to admin_mission_path(@mission.slug), notice: "Step updated."
    end

    def destroy
      @step.update!(deleted_at: Time.current)
      redirect_to admin_mission_path(@mission.slug), notice: "Step removed."
    end
